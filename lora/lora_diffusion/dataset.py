...
STYLE_TEMPLATE = [
    ...
    "write this in the masked region: {}"
    "written text: {}",
    "engraved {}",
    "a street sign showing {}",
    "spray-painted {} on the wall",
    "a poster that says {}",
    "billboard displaying {}",
    "handwritten {}",
    "neon sign glowing with {}",
    "chalkboard with {}",
    "LED display showing {}",
    "road marking of {}",
    "painted {} on a surface",
    "a warning sign: {}",
    "digital screen showing {}",
    "a label with {}",
    "scribbled {}",
    "etched {} into metal",
    "banner that reads {}",
...
class PivotalTuningDatasetCapation(Dataset):
    ...
    def __init__(
        ...
        if use_mask_captioned_data:
            print("Mask captioned data is available.")
            src_imgs = glob.glob(str(instance_data_root) + "/*_src.png")
            for f in src_imgs:
                idx = int(str(Path(f).stem).split("_")[0])
                mask_path = f"{instance_data_root}/{idx}_mask.png"
            ...
            self.captions = open(f"{instance_data_root}/modified_captions.txt").readlines()
